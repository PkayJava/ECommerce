...
public class Select2MultipleChoice<T> extends FormComponent<Collection<T>> implements IRequestListener {
    ...
    public void renderHead(IHeaderResponse response) {
        ...
        response.render(OnDomReadyHeaderItem.forScript("$('#" + markupId + "')." + "select2({ ajax: { url: '" + url + "', dataType: 'json', delay: 250, data: function (params) { return { q: params.term, page: params.page }; }, processResults: function (data, params) { params.page = params.page || 1; return { results: data.items, pagination: { more: data.more } }; }, cache: true }, escapeMarkup: function (markup) { return markup; }, minimumInputLength: 1});"));
    }

    @Override
    protected Collection<T> convertValue(String[] value) throws ConversionException {
        if (value != null && value.length > 0) {
            return this.provider.toChoices(Arrays.asList(value));
        } else {
            return null;
        }
