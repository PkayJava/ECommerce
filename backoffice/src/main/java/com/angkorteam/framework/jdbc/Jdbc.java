...
public class Jdbc {
    ...
    public String insert(String table, Map<String, Object> values, Map<String, Object> systems) throws SQLException {
        SelectQuery selectQuery = new SelectQuery(INSTALL);
        selectQuery.addWhere("`table` = :table", table);
        Install install = this.named.queryForObject(selectQuery.toSQL(), selectQuery.getParam(), Install.class);
        if (install == null) {
...
//                }
                ...
                for (Map.Entry<String, Object> attr : values.entrySet()) {
                    insertQuery.addValue(AFTER + "_" + attr.getKey() + " = :" + AFTER + "_" + attr.getKey(), attr.getValue());
                }
                insertQuery.addValue(this.userPk + " = :" + this.userPk, systems.get(this.userPk));
                insertQuery.addValue("dt_when = :dt_when", new Date());
                insertQuery.addValue("op = :op", "INSERT");
                insertQuery.addValue(pk + " = :" + pk, id);
                ...
                this.named.update(insertQuery.toSQL(), insertQuery.getParam());
            }
            InsertQuery insertQuery = new InsertQuery(table);
            for (Map.Entry<String, Object> attr : values.entrySet()) {
                insertQuery.addValue(attr.getKey() + " = :" + attr.getKey(), attr.getValue());
            }
            insertQuery.addValue(pk + " = :" + pk, id);
            this.named.update(insertQuery.toSQL(), insertQuery.getParam());
            return null;
...
//                param.put(Engine.SYSTEM_TRANSACTION_ID, systems.get(Engine.SYSTEM_TRANSACTION_ID));
            ...
            for (Map.Entry<String, Object> attr : wheres.entrySet()) {
                deleteQuery.addWhere(attr.getKey() + " = :" + attr.getKey(), attr.getValue());
            }
            this.named.update(deleteQuery.toSQL(), deleteQuery.getParam());
            return null;
        } else if (install.getLevel() == LEVEL_DRAFT) {
            if (systems.get(this.userPk) == null || "".equals(systems.get(this.userPk))) {
                throw new SQLException(this.userPk + " is required");
...
//                param.put(Engine.SYSTEM_TRANSACTION_ID, systems.get(Engine.SYSTEM_TRANSACTION_ID));
            ...
            for (Map.Entry<String, Object> attr : values.entrySet()) {
                updateQuery.addValue(attr.getKey() + " = :" + attr.getKey(), attr.getValue());
            }
            for (Map.Entry<String, Object> attr : wheres.entrySet()) {
                updateQuery.addWhere(attr.getKey() + " = :" + attr.getKey(), attr.getValue());
            }
            this.named.update(updateQuery.toSQL(), updateQuery.getParam());
...
//            param.put(SYSTEM_TRANSACTION_ID, systems.get(SYSTEM_TRANSACTION_ID));
    ...
    public void draftReject(String taskId, String reason, Map<String, Object> systems) throws SQLException {
        SelectQuery selectQuery = new SelectQuery(TASK);
        selectQuery.addWhere("task_id = :task_id", taskId);
        Task task = this.named.queryForObject(selectQuery.toSQL(), selectQuery.getParam(), Task.class);
        if (task == null) {
            throw new SQLException("task " + taskId + " is not found");
        }
        selectQuery = new SelectQuery(INSTALL);
        ...
        selectQuery = new SelectQuery(draftTable);
        selectQuery.addWhere(draftPk + " = :" + draftPk, task.getTaskId());
        Map<String, Object> record = this.named.queryForMap(selectQuery.toSQL(), selectQuery.getParam());
        if (record == null) {
            throw new SQLException("task " + taskId + " is not found");
        }
        ...
        selectQuery = new SelectQuery(draftSummaryTable);
        selectQuery.addField("COUNT(*)");
        selectQuery.addWhere("action = :action", "NO");
        selectQuery.addWhere(draftPk + " = :" + draftPk, task.getTaskId());
        int runDraftNo = this.named.queryForObject(selectQuery.toSQL(), selectQuery.getParam(), int.class);
    ...
    public void draftApprove(String taskId, Map<String, Object> systems) throws SQLException {
        ...
        selectQuery.addWhere(draftPk + " = :" + draftPk, task.getTaskId());
        int runDraftYes = this.named.queryForObject(selectQuery.toSQL(), selectQuery.getParam(), int.class);
    ...
    public void reviewReject(String taskId, String reason, Map<String, Object> systems) throws SQLException {
        ...
        selectQuery = new SelectQuery(reviewTable);
        ...
        selectQuery = new SelectQuery(reviewSummaryTable);
        selectQuery.addField("COUNT(*)");
        selectQuery.addWhere("action = :action", "NO");
        selectQuery.addWhere(reviewPk + " = :" + reviewPk, task.getTaskId());
        int runReviewNo = this.named.queryForObject(selectQuery.toSQL(), selectQuery.getParam(), int.class);
    ...
    public void reviewApprove(String taskId, Map<String, Object> systems) throws SQLException {
        ...
        selectQuery.addWhere("action = :action", "YES");
        selectQuery.addWhere(reviewPk + " = :" + reviewPk, task.getTaskId());
        int runReviewYes = this.named.queryForObject(selectQuery.toSQL(), selectQuery.getParam(), int.class);
        ...
        if (runReviewYes >= reviewYes) {
            ...
            if ("INSERT".equals(op)) {
                ...
                selectQuery = new SelectQuery(reviewTable);
                selectQuery.addWhere(reviewPk + " = :" + reviewPk, task.getTaskId());
                Map<String, Object> from = this.named.queryForMap(selectQuery.toSQL(), selectQuery.getParam());

                updateQuery = new UpdateQuery(table);
                updateQuery.addWhere(pk + " = :" + pk, record.get(pk));
                for (String intoField : intoFields) {
                    updateQuery.addValue(intoField + " = :" + intoField, from.get(AFTER + "_" + intoField));
                }
                this.named.update(updateQuery.toSQL(), updateQuery.getParam());
            } else if ("DELETE".equals(op)) {
                DeleteQuery deleteQuery = new DeleteQuery(table);
                deleteQuery.addWhere(pk + " = :" + pk, record.get(pk));
                this.named.update(deleteQuery.toSQL(), deleteQuery.getParam());
            }
